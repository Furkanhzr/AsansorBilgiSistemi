...
@section('title-page','Arıza Kontrol Talebi Oluştur')
@section('content')
    <style>
        .ck-editor__editable
        {
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            ...
            <h3  style="color: black"><strong>Arıza Kontrol Talebi Oluşturma</strong></h3>
            <hr style="color: #F28123; width: 430px;height: 2px;">
        </div>
        <div class="card-body">
            @if($errors->any())
            ...
            @endif
            <form method="POST" action="{{route('faultRequest.create')}}" enctype="multipart/form-data">
                @csrf
                <div class="form-group">
                    <select class="form-control" name="elevator" id="elevator">
                        <option selected disabled>Seçim Yapınız</option>
                        @foreach($elevators as $elevator)
                            <option value="{{$elevator->id}}">{{$elevator->key_code}}</option>
                        @endforeach
                    </select>
                </div>
